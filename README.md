...
**NOTE**: *You can call your geonode project whatever you like following the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``geocem`` with whatever you would like to name your project.*
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geocem/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geocem sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geocem/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geocem/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geocem sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geocem/br/restore.sh $BKP_FOLDER_NAME'
```
